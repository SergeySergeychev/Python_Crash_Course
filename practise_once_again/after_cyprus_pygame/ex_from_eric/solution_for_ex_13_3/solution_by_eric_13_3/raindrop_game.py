...
class RaindropsGame:
    ...
    def _create_drops(self):
        ...
        # Detetrmine the number of rows of drops that fit on the screen.
    ...
    def _update_raindrops(self):
        """Update drop position, and look for drops
        ...
        for drop in self.raindrops.copy():
            if drop.check_disappeared():
                # Remove this drop and make new row.
                self.raindrops.remove(drop)
                make_new_drops = True
        # Make a new rof of drops if needed.
        if make_new_drops:
            self._create_row(0)
